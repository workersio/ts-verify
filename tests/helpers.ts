...
function helper(
  expected: 'verified' | 'unverified' | 'incorrect' | 'timeout',
  description: string,
  debug: boolean,
  expectedModel: Map<FreeVar, any>
): Mocha.Test {
  const body = async () => {
    ...
    const foo = expect(vc).to.not.be.undefined;
...
export function skip(description: string) {
  return it.skip(description);
}

interface VerifiedFun {
  (description: string): Mocha.Test;
  debug: (description: string) => Mocha.Test;
}
interface UnverifiedFun {
  (description: string, ...expectedVariables: Array<[FreeVar, any]>): Mocha.Test;
  debug: (description: string, ...expectedVariables: Array<[FreeVar, any]>) => Mocha.Test;
}

export const verified: VerifiedFun = (() => {
  const f: any = (description: string): Mocha.Test => helper('verified', description, false, new Map());
  f.debug = (description: string): Mocha.Test => helper('verified', description, true, new Map());
  return f;
})();

export const unverified: UnverifiedFun = (() => {
  const f: any = (description: string, ...expectedVariables: Array<[FreeVar, any]>): Mocha.Test =>
    helper('unverified', description, false, new Map(expectedVariables));
  f.debug = (description: string, ...expectedVariables: Array<[FreeVar, any]>): Mocha.Test =>
...
export const timeout: VerifiedFun = (() => {
  const f: any = (description: string): Mocha.Test => helper('timeout', description, false, new Map()).timeout(6000);
  f.debug = (description: string): Mocha.Test => helper('timeout', description, true, new Map());
  return f;
})();
...
function bisimulationHelper(description: string, fn: () => any, steps: number, debug: boolean) {
  const testBody = () => {
    ...
    if (evalError === undefined) {
      const foo = expect(interpreterError).to.be.undefined;
    } else {
...
interface BisimulationFun {
  (description: string, code: () => any, steps: number): Mocha.Test;
  debug: (description: string, code: () => any, steps: number) => Mocha.Test;
}

export const bisumulate: BisimulationFun = (() => {
  const f: any = (description: string, code: () => any, steps: number): Mocha.Test =>
    bisimulationHelper(description, code, steps, false);
  f.debug = (description: string, code: () => any, steps: number): Mocha.Test =>
