...
import {
  ...
  nullLoc,
  removeTestCodePrefix,
  replaceVarAssertion as replaceJSVarAssertion,
  replaceVarFunction as replaceJSVarFunction,
  replaceVarBlock as replaceJSVarBlock,
  uniqueIdentifier,
  eqSourceLocation,
  compEndPosition
} from './javascript';
import {
  ...
  or,
  removePrefix,
  replaceResultWithCall,
  replaceVar,
  transformClassInvariant,
  transformEveryInvariant,
  transformSpec,
  tru,
  truthy,
  und,
...
export class VCGenerator extends Visitor<
  [A, AccessTriggers, Syntax.Expression],
...
> {
  ...
  verify(
    ...
    const assumptions: Array<Assumption> = this.assumptions.map(
      (source: string): Assumption => ({ source, prop: tru, canBeDeleted: false })
    );
    this.vcs.push(
  ...
  compareTypePropAndTest(
    exprA: A,
    exprE: Syntax.Expression,
    type: 'boolean' | 'number' | 'string' | 'number or string' | 'int'
  ): [P, Syntax.Expression] {
    switch (type) {
      case 'boolean':
        return [compareType(exprA, 'boolean'), this.compareType(exprE, 'boolean')];
      case 'number':
        return [compareType(exprA, 'number'), this.compareType(exprE, 'number')];
      case 'string':
        return [compareType(exprA, 'string'), this.compareType(exprE, 'string')];
  ...
  visitOldIdentifierTerm(term: Syntax.OldIdentifier): [A, AccessTriggers, Syntax.Expression] {
    if (!isMutable(term.id)) {
      throw new Error('not mutable');
    }
    return [{ type: 'HeapReference', heap: this.oldHeap, loc: term.id.name }, [], id(`old_${term.id.name}`, term.loc)];
  }
  ...
  insertWrapper(
    ...
    loc: Syntax.SourceLocation,
    args: Array<Syntax.Identifier>,
    rT: TestCode,
    retName: Syntax.Identifier,
    sT: TestCode
  ): Syntax.Expression {
    return {
      type: 'CallExpression',
  ...
  visitSpecAssertion(assertion: Syntax.SpecAssertion): [P, AccessTriggers, Syntax.Expression, TestCode] {
    ...
    const sP3 = replaceResultWithCall(
      calleeA,
      this.heap + 1,
      thisArg,
      assertion.args,
      assertion.post.argument && assertion.post.argument.name,
      sP2
    );
    const specP = transformSpec(calleeA, thisArg, assertion.args, rP2, sP3, this.heap + 1);
    const retName =
      assertion.post.argument === null ? id(`_res_${uniqueIdentifier(assertion.loc)}`) : assertion.post.argument;
      ...
      if (rT.length + sT.length > 0) {
        specT.push({
          ...
          expression: {
            type: 'AssignmentExpression',
            left: calleeE,
            right: this.insertWrapper(
              calleeE,
              assertion.loc,
              assertion.args.map((a) => id(a)),
              rT,
              retName,
              sT
            ),
        ...
        args: [
          {
            ...
            id: null,
            params:
              assertion.indexArgument !== null ? [assertion.argument, assertion.indexArgument] : [assertion.argument],
            requires: [],
            ensures: [],
            body: {
              type: 'BlockStatement',
  ...
  visitArrayExpression(expr: Syntax.ArrayExpression): [A, Syntax.Expression] {
    ...
    elems.forEach((property, idx) => {
      this.have(
        eq({ type: 'ArrayIndexExpression', array: object, index: { type: 'Literal', value: idx } }, elems[idx])
      );
    });
  ...
  visitObjectExpression(expr: Syntax.ObjectExpression): [A, Syntax.Expression] {
    ...
    expr.properties.forEach(({ key }, idx) => {
      this.have(eq({ type: 'MemberExpression', object, property: { type: 'Literal', value: key } }, values[idx]));
    });
    return [
      object,
      {
  ...
  visitFunction(
    ...
    const post: Array<[P, AccessTriggers, TestCode]> = fun.ensures.map((ens) => {
      ...
      if (ens.argument !== null) {
        ens2 = replaceJSVarAssertion(
          ens.argument.name,
          id(retVar, ens.argument.loc),
          id(retVar, ens.argument.loc),
          ens2
        );
      }
    ...
    const inlinedSpec = transformSpec(
      funcName,
      thisArg,
      args,
      preP,
  ...
  visitClassDeclaration(stmt: Syntax.ClassDeclaration): BreakCondition {
    ...
    for (const method of stmt.methods) {
      ...
      inlinedMethodTestBodies.push({
        ...
        ensures: [],
        body:
          preTestCode.length + postTestCode.length === 0
            ? blockStmt
            : {
                type: 'BlockStatement',
                body: [
  ...
  classDeclarationCode(
    clsDef: Syntax.ClassDeclaration,
    methods: Array<Syntax.MethodDeclaration>,
    invT: TestCode
  ): TestCode {
