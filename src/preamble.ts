...
class PreambleGenrator extends VCGenerator {
  verify(_vc: P, _testBody: TestCode, _loc: Syntax.SourceLocation, _desc: string) {
    /* only generate preamble, do not verify */
  }

  createFunctionBodyInliner() {
    return new PreambleGenrator(
      this.classes,
      this.heap + 1,
      this.heap + 1,
      new Set([...this.locs]),
...
export function generatePreamble(): Preamble {
  if (cachedPreamble === null) {
    ...
    const vcgen = new PreambleGenrator(new Set(), 0, 0, new Set(), new Set(), [], [], true, tru);
