...
class Stringifier extends Visitor<string, string, string, string> {
  ...
  visitConditionalTerm(term: Syntax.ConditionalTerm): string {
    return (
      `(${this.visitTerm(term.test)} ? ${this.visitTerm(term.consequent)} ` + `: ${this.visitTerm(term.alternate)})`
    );
  }
  ...
  visitSpecAssertion(assertion: Syntax.SpecAssertion): string {
    if (assertion.post.argument) {
      return (
        `spec(${this.visitTerm(assertion.callee)}, ` +
  ...
  visitEveryAssertion(assertion: Syntax.EveryAssertion): string {
    if (assertion.indexArgument !== null) {
      return (
        `${this.visitTerm(assertion.array)}.every(` +
  ...
  visitPureAssertion(_assertion: Syntax.PureAssertion): string {
    return `pure()`;
  }
  ...
  visitConditionalExpression(expr: Syntax.ConditionalExpression): string {
    return (
      `(${this.visitExpression(expr.test)} ? ${this.visitExpression(expr.consequent)} ` +
  ...
  visitObjectExpression(expr: Syntax.ObjectExpression): string {
    ...
    return `{ ${expr.properties
      .map(({ key, value }) => `${nameToKey(key)}: ${this.visitExpression(value)}`)
      .join(', ')} }`;
  }
  ...
  visitFunctionExpression(expr: Syntax.FunctionExpression): string {
    if (
      ...
      expr.body.body[0].type === 'ReturnStatement' &&
      expr.requires.length === 0 &&
      expr.ensures.length === 0
    ) {
    ...
    return (
      `(function ${expr.id ? expr.id.name + ' ' : ''}(${expr.params.map((p) => p.name).join(', ')}) ` +
  ...
  visitIfStatement(stmt: Syntax.IfStatement): string {
    if (stmt.alternate.body.length === 0) {
      return this.i(`if (${this.visitExpression(stmt.test)}) ` + `${this.formatBlock(stmt.consequent.body)}\n`);
    } else {
      return this.i(
        `if (${this.visitExpression(stmt.test)}) ` +
  ...
  visitDebuggerStatement(_stmt: Syntax.DebuggerStatement): string {
  ...
  visitFunctionDeclaration(stmt: Syntax.FunctionDeclaration): string {
    ...
    return this.i(
      `function ${stmt.id.name} (${stmt.params.map((p) => p.name).join(', ')}) ` + `${this.formatBlock(body)}\n`
    );
  }
  ...
  visitClassDeclaration(stmt: Syntax.ClassDeclaration): string {
    ...
    stmt.methods.forEach((method) => {
      ...
      res += this.i(
        `${method.id.name} (${method.params.map((p) => p.name).join(', ')}) ` + `${this.formatBlock(body)}\n`
      );
    });
    this.depth--;
    res += this.i(`}\n`);
