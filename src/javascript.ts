...
export namespace Syntax {
  ...
  export interface OldIdentifier {
    type: 'OldIdentifier';
    id: Identifier;
    loc: SourceLocation;
  }
  ...
  export interface IsIntegerTerm {
    type: 'IsIntegerTerm';
    term: Term;
    loc: SourceLocation;
  }
  export interface ToIntegerTerm {
    type: 'ToIntegerTerm';
    term: Term;
    loc: SourceLocation;
  }
  ...
  export interface EveryAssertion {
    type: 'EveryAssertion';
    array: Term;
    argument: Identifier;
  ...
  export interface InstanceOfAssertion {
    type: 'InstanceOfAssertion';
    left: Term;
    right: Identifier;
    loc: SourceLocation;
  }
  export interface InAssertion {
    type: 'InAssertion';
    property: Term;
    object: Term;
    loc: SourceLocation;
  }
  export interface UnaryAssertion {
    type: 'UnaryAssertion';
    operator: '!';
    argument: Assertion;
    loc: SourceLocation;
  ...
  export type Program = { body: Array<Statement>; invariants: Array<Assertion> };
}
...
export function eqEndPosition(loc1: Syntax.SourceLocation, loc2: Syntax.SourceLocation): boolean {
  return loc1.file === loc2.file && loc1.end.line === loc2.end.line && loc1.end.column === loc2.end.column;
}
...
export abstract class Visitor<T, A, E, S> {
  ...
  visitTerm(term: Syntax.Term): T {
    switch (term.type) {
      case 'Identifier':
        return this.visitIdentifierTerm(term);
      case 'OldIdentifier':
        return this.visitOldIdentifierTerm(term);
      case 'Literal':
        return this.visitLiteralTerm(term);
      case 'UnaryTerm':
        return this.visitUnaryTerm(term);
      case 'BinaryTerm':
        return this.visitBinaryTerm(term);
      case 'LogicalTerm':
        return this.visitLogicalTerm(term);
      case 'ConditionalTerm':
        return this.visitConditionalTerm(term);
      case 'CallTerm':
        return this.visitCallTerm(term);
      case 'MemberTerm':
        return this.visitMemberTerm(term);
      case 'IsIntegerTerm':
        return this.visitIsIntegerTerm(term);
      case 'ToIntegerTerm':
        return this.visitToIntegerTerm(term);
    }
  }
  ...
  visitAssertion(assertion: Syntax.Assertion): A {
    switch (assertion.type) {
      case 'PureAssertion':
        return this.visitPureAssertion(assertion);
      case 'SpecAssertion':
        return this.visitSpecAssertion(assertion);
      case 'EveryAssertion':
        return this.visitEveryAssertion(assertion);
      case 'InstanceOfAssertion':
        return this.visitInstanceOfAssertion(assertion);
      case 'InAssertion':
        return this.visitInAssertion(assertion);
      case 'UnaryAssertion':
        return this.visitUnaryAssertion(assertion);
      case 'BinaryAssertion':
        return this.visitBinaryAssertion(assertion);
      default:
        return this.visitTermAssertion(assertion);
    }
  }
  ...
  visitExpression(expr: Syntax.Expression): E {
    switch (expr.type) {
      ...
      case 'LogicalExpression':
        return this.visitLogicalExpression(expr);
      case 'ConditionalExpression':
        return this.visitConditionalExpression(expr);
      case 'AssignmentExpression':
        return this.visitAssignmentExpression(expr);
      case 'SequenceExpression':
        return this.visitSequenceExpression(expr);
      case 'CallExpression':
        return this.visitCallExpression(expr);
      case 'NewExpression':
        return this.visitNewExpression(expr);
      case 'ArrayExpression':
        return this.visitArrayExpression(expr);
      case 'ObjectExpression':
        return this.visitObjectExpression(expr);
      case 'InstanceOfExpression':
        return this.visitInstanceOfExpression(expr);
      case 'InExpression':
        return this.visitInExpression(expr);
  ...
  visitStatement(stmt: Syntax.Statement): S {
    switch (stmt.type) {
      ...
      case 'AssertStatement':
        return this.visitAssertStatement(stmt);
      case 'IfStatement':
      ...
      case 'ClassDeclaration':
        return this.visitClassDeclaration(stmt);
    }
  }
...
export function id(
  name: string,
  loc: Syntax.SourceLocation = nullLoc(),
...
class Traverser extends Visitor<void, void, void, void> {
  visitIdentifierTerm(_term: Syntax.Identifier): void {
    /* empty */
  }
  ...
  visitLiteralTerm(_term: Syntax.Literal): void {
    /* empty */
  }
  ...
  visitPureAssertion(_assertion: Syntax.PureAssertion): void {
    /* empty */
  }
  ...
  visitIdentifier(_expr: Syntax.Identifier): void {
    /* empty */
  }

  visitLiteral(_expr: Syntax.Literal): void {
    /* empty */
  }
  ...
  visitDebuggerStatement(_stmt: Syntax.DebuggerStatement): void {
    /* empty */
  }
...
export class Substituter extends Visitor<Syntax.Term, Syntax.Assertion, Syntax.Expression, Syntax.Statement> {
  ...
  withoutBindings<A>(cont: () => A, ...bindings: Array<string>): A {
    ...
    try {
      bindings.forEach((b) => {
        delete this.thetaA[b];
        delete this.thetaE[b];
      });
      return cont();
    } finally {
  ...
  visitEveryAssertion(assertion: Syntax.EveryAssertion): Syntax.Assertion {
    return {
      ...
      expression:
        assertion.indexArgument !== null
          ? this.withoutBindings(
              () => this.visitAssertion(assertion.expression),
              assertion.argument.name,
              assertion.indexArgument.name
            )
  ...
  visitFunctionExpression(expr: Syntax.FunctionExpression): Syntax.FunctionExpression {
    const bindings =
      expr.id !== null ? [expr.id.name, ...expr.params.map((p) => p.name)] : expr.params.map((p) => p.name);
    return {
      type: 'FunctionExpression',
      id: expr.id,
      params: expr.params,
      requires: expr.requires.map((req) => this.withoutBindings(() => this.visitAssertion(req), ...bindings)),
      ensures: expr.ensures.map((ens) => this.withoutBindings(() => this.visitPostCondition(ens), ...bindings)),
  ...
  visitFunctionDeclaration(stmt: Syntax.FunctionDeclaration): Syntax.FunctionDeclaration {
    ...
    return {
      ...
      id: stmt.id,
      params: stmt.params,
      requires: stmt.requires.map((req) => this.withoutBindings(() => this.visitAssertion(req), ...bindings)),
      ensures: stmt.ensures.map((ens) => this.withoutBindings(() => this.visitPostCondition(ens), ...bindings)),
...
export function replaceVarAssertion(
  varName: string,
  substA: Syntax.Term,
  substE: Syntax.Expression,
  assertion: Syntax.Assertion
): Syntax.Assertion {
...
export function replaceVarExpr(
  varName: string,
  substA: Syntax.Term,
  substE: Syntax.Expression,
...
export function replaceVarFunction(
  varName: string,
  substA: Syntax.Term,
  substE: Syntax.Expression,
  f: Syntax.Function
  ...
  switch (f.type) {
    case 'FunctionDeclaration':
      return sub.visitFunctionDeclaration(f);
    case 'FunctionExpression':
      return sub.visitFunctionExpression(f);
    case 'MethodDeclaration':
      return sub.visitMethodDeclaration(f);
  }
}

export function replaceVarStmt(
  varName: string,
  substA: Syntax.Term,
  substE: Syntax.Expression,
...
export function replaceVarBlock(
  varName: string,
  substA: Syntax.Term,
  substE: Syntax.Expression,
...
export function uniqueIdentifier(loc: Syntax.SourceLocation): number {
  return loc.start.column + loc.start.line * 37 + loc.end.column * 331 + loc.end.line * 5023 + loc.file.length * 48353;
}
...
class SourceLocationFinder extends Traverser {
  ...
  matchLocation(loc: Syntax.SourceLocation): void {
    ...
    if (this.bestSourceLocation === null) {
      ...
      const prevScore =
        (this.bestSourceLocation.end.line - this.bestSourceLocation.start.line) * 120 +
        this.bestSourceLocation.end.column -
        this.bestSourceLocation.start.column;
