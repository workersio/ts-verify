...
function assignUpdate(
  ...
  right: JSyntax.Expression,
  expr: JSyntax.Expression,
  ...
  switch (expr.type) {
      ...
      return {
        ...
        value:
          typeof expr.value === 'bigint'
            ? (() => {
                throw unsupported(expr, 'bigint literals are not supported');
              })()
            : (expr.value as undefined | null | boolean | number | string),
        loc: loc(expr)
      };
    case 'ThisExpression':
    ...
    case 'BinaryExpression': {
      return {
        ...
        left: expressionAsTerm((expr as any).left),
    ...
    case 'CallExpression':
      if (expr.callee.type === 'Identifier' && expr.callee.name === 'old') {
      ...
      if (
        ...
        expr.callee.property.name === 'isInteger' &&
      ...
      if (
        ...
        !expr.callee.computed &&
        expr.callee.property.type === 'Identifier' &&
        expr.callee.property.name === 'trunc' &&
        expr.arguments.length === 1
      ) {
        const arg = expr.arguments[0];
        if (arg.type === 'SpreadElement') {
    ...
    case 'MemberExpression':
      ...
      if (expr.computed) {
        property = expressionAsTerm(expr.property as JSyntax.Expression);
      } else {
  ...
  switch (expr.type) {
    ...
    case 'BinaryExpression': {
      ...
      if (expr.operator === 'in') {
        return {
          type: 'InAssertion',
          property: expressionAsTerm((expr as any).left),
    ...
    case 'CallExpression':
      ...
      if (expr.callee.type === 'Identifier' && expr.callee.name === 'spec') {
        ...
        if (
          r.params.length < s.params.length - 1 ||
      ...
      if (
        ...
        if (inv.params.length < 1 || inv.params.length > 2 || inv.params.some((p, _idx) => p.type !== 'Identifier')) {
  ...
  switch (expr.type) {
    ...
    case 'BinaryExpression': {
      if (expr.operator === 'instanceof') {
        ...
        return {
          ...
          left: expressionAsJavaScript((expr as any).left, withAssertions),
          right: patternAsIdentifier(expr.right),
          loc: loc(expr)
        };
      }
      if (expr.operator === 'in') {
        return {
          type: 'InExpression',
          property: expressionAsJavaScript((expr as any).left, withAssertions),
    ...
    case 'AssignmentExpression':
      ...
      switch (expr.operator) {
        ...
        case '+=':
          return assignUpdate(to, '+', expr.right, expr, withAssertions);
        case '-=':
          return assignUpdate(to, '-', expr.right, expr, withAssertions);
        case '*=':
          return assignUpdate(to, '*', expr.right, expr, withAssertions);
    ...
    case 'ArrayExpression':
      return {
        ...
        elements: expr.elements.map((expr) => {
          if (expr !== null && expr.type === 'SpreadElement') {
            throw unsupported(expr);
          } else {
            return expr !== null ? expressionAsJavaScript(expr, withAssertions) : (undefined as any);
          }
        }),
        loc: loc(expr)
      };
    ...
    case 'MemberExpression':
      ...
      if (expr.computed) {
        property = expressionAsJavaScript(expr.property as JSyntax.Expression, withAssertions);
      } else {
        if (expr.property.type !== 'Identifier') throw unsupported(expr.property);
        property = { type: 'Literal', value: expr.property.name, loc: loc(expr.property) };
      }
    ...
    case 'ArrowFunctionExpression':
      ...
      const fe: Syntax.FunctionExpression = {
        ...
        body: {
          type: 'BlockStatement',
          body: withAssertions
            ? flatMap(withoutPseudoCalls('requires', withoutPseudoCalls('ensures', body)), (stmt) =>
                statementAsJavaScript(stmt, true)
              )
...
function findClassInvariant(clsDef: JSyntax.ClassDeclaration): JSyntax.Expression {
  ...
  for (const method of clsDef.body.body) {
    if ((method as any).key?.type === 'Identifier' && (method as any).key?.name === 'invariant') {
      if (invMethod !== null) {
        throw unsupported(method, 'class can have at most one invariant');
      }
      invMethod = method as JSyntax.MethodDefinition;
    }
  }
  if (invMethod === null) {
...
function statementAsJavaScript(stmt: JSyntax.Statement, withAssertions: boolean): Array<Syntax.Statement> {
  function assert(cond: boolean) {
    if (!cond) throw unsupported(stmt);
  }
  switch (stmt.type) {
    case 'EmptyStatement':
      return [];
      ...
      return [
        {
          ...
          argument: stmt.argument
            ? expressionAsJavaScript(stmt.argument, withAssertions)
            : { type: 'Literal', value: undefined, loc: loc(stmt) },
          loc: loc(stmt)
        }
      ];
    case 'FunctionDeclaration': {
      ...
      const fd: Syntax.FunctionDeclaration = {
        ...
        body: {
          type: 'BlockStatement',
          body: withAssertions
            ? flatMap(withoutPseudoCalls('requires', withoutPseudoCalls('ensures', body)), (s) =>
                statementAsJavaScript(s, true)
              )
    ...
    case 'ClassDeclaration': {
      ...
      for (const method of stmt.body.body) {
        if (method.type === 'PropertyDefinition' || method.type === 'StaticBlock') continue;
        ...
        methods.push({
          ...
          body: {
            type: 'BlockStatement',
            body: withAssertions
              ? flatMap(withoutPseudoCalls('requires', withoutPseudoCalls('ensures', func.body.body)), (s) =>
                  statementAsJavaScript(s, true)
                )
