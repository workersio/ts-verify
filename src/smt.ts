...
class SMTGenerator extends Visitor<SMTInput, SMTInput, SMTInput, SMTInput> {
  ...
  visitHeapEffect(expr: Syntax.HeapEffect): SMTInput {
    ...
    return (
      `(eff${args.length} ${this.visitExpr(callee)} ${this.visitHeapExpr(heap)} ` +
  ...
  visitCallExpression(expr: Syntax.CallExpression): SMTInput {
    const { callee, heap, thisArg, args } = expr;
    return (
      `(app${args.length} ${this.visitExpr(callee)} ${this.visitHeapExpr(heap)} ` +
      `${this.visitExpr(thisArg)}${args.map((a) => ' ' + this.visitExpr(a)).join('')})`
    );
  }
  ...
  visitAnd(prop: Syntax.And): SMTInput {
    const clauses: Array<SMTInput> = flatMap(prop.clauses, (c) => (c.type === 'And' ? c.clauses : [c]))
  ...
  visitOr(prop: Syntax.Or): SMTInput {
    const clauses: Array<SMTInput> = flatMap(prop.clauses, (c) => (c.type === 'Or' ? c.clauses : [c]))
      .map((p) => this.visitProp(p))
      .filter((s) => s !== `false`);
  ...
  visitTrue(_prop: Syntax.True): SMTInput {
    return `true`;
  }

  visitFalse(_prop: Syntax.False): SMTInput {
    return `false`;
  }
  ...
  visitPrecondition(prop: Syntax.Precondition): SMTInput {
    ...
    return (
      `(pre${args.length} ${this.visitExpr(callee)} ${this.visitHeapExpr(heap)} ` +
      `${this.visitExpr(thisArg)}${args.map((a) => ' ' + this.visitExpr(a)).join('')})`
    );
  }
  ...
  visitPostcondition(prop: Syntax.Postcondition): SMTInput {
    const { callee, heap, thisArg, args } = prop;
    return (
      `(post${args.length} ${this.visitExpr(callee)} ${this.visitHeapExpr(heap)} ` +
      `${this.visitExpr(thisArg)}${args.map((a) => ' ' + this.visitExpr(a)).join('')})`
    );
  }
  ...
  visitForAllCalls(prop: Syntax.ForAllCalls): SMTInput {
    ...
    if (prop.existsLocs.size + prop.existsHeaps.size + prop.existsVars.size > 0) {
      p =
        `(exists (${[...prop.existsHeaps].map((h) => `(${this.visitHeap(h)} Heap)`).join(' ')} ` +
    ...
    return (
      `(forall ((${this.visitHeap(heap)} Heap) ` +
  ...
  visitCallTrigger(prop: Syntax.CallTrigger): SMTInput {
    const { callee, heap, thisArg, args } = prop;
    return (
      `(call${args.length} ${this.visitExpr(callee)} ${this.visitHeapExpr(heap)} ` +
      `${this.visitExpr(thisArg)}${args.map((a) => ' ' + this.visitExpr(a)).join('')})`
    );
  }
  ...
  visitForAllAccessObject(prop: Syntax.ForAllAccessObject): SMTInput {
    ...
    return (
      `(forall ((${this.visitVariable(prop.thisArg)} JSVal) ` +
  ...
  visitForAllAccessProperty(prop: Syntax.ForAllAccessProperty): SMTInput {
    ...
    const trigger: SMTInput = this.visitProp(accessP);
    return (
      `(forall ((${this.visitVariable(prop.property)} JSVal) ` +
      `(${this.visitHeap(heap)} Heap)) (!\n  ${p}\n  :pattern ${trigger}))`
    );
  }
...
${[...classes].map(({ cls }) => `  (ite (is-jsobj_${cls} x) "[object ${cls}]"`).join('\n')}
  "[object Object]"${[...classes].map((_c) => ')').join('')}))))))))
...
${[...classes]
  .map(
    ({ cls }) =>
      ...
      (and (is-jsobj_${cls} obj) (= cls c_${cls}))`
  )
  .join('\n')}
))

; Methods
${flatMap([...classes], ({ cls, methods }) => methods.map((method) => ({ cls, method })))
  .map(({ cls, method }) => `(declare-fun v_${cls}.${method} () JSVal)\n`)
  .join('')}
...
${strClass.methods.map((method) => `      (and (is-jsstr obj) (= (_tostring prop) "${method}"))`).join('\n')}
...
${flatMap([...classes], ({ cls, fields }) => fields.map((field) => ({ cls, field })))
  .map(({ cls, field }) => `      (and (is-jsobj_${cls} obj) (= (_tostring prop) "${field}"))`)
  .join('\n')}
${flatMap([...classes], ({ cls, methods }) => methods.map((method) => ({ cls, method })))
  .map(({ cls, method }) => `      (and (is-jsobj_${cls} obj) (= (_tostring prop) "${method}"))`)
  .join('\n')}
))
...
${strClass.methods
  .map((method) => `  (ite (and (is-jsstr obj) (= (_tostring prop) "${method}")) v_String.${method}`)
  .join('\n')}
...
${flatMap([...classes], ({ cls, fields }) => fields.map((field) => ({ cls, field })))
  .map(({ cls, field }) => `  (ite (and (is-jsobj_${cls} obj) (= (_tostring prop) "${field}")) (${cls}-${field} obj)`)
  .join('\n')}
${flatMap([...classes], ({ cls, methods }) => methods.map((method) => ({ cls, method })))
  .map(({ cls, method }) => `  (ite (and (is-jsobj_${cls} obj) (= (_tostring prop) "${method}")) v_${cls}.${method}`)
  .join('\n')}
  jsundefined
