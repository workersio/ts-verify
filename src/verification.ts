...
export default class VerificationCondition {
  ...
  runTest(): Message {
    ...
    try {
      ...
      if (e instanceof Error && (e instanceof TypeError || e.message === 'assertion failed')) {
        ...
        return unexpected(e instanceof Error ? e : new Error(String(e)), this.loc, this.description);
      }
    }
  }
  ...
  runWithInterpreter(): Message {
    ...
    try {
      ...
      if (e instanceof Error && (e instanceof TypeError || e.message === 'assertion failed')) {
        ...
        return (this.result = unexpected(e instanceof Error ? e : new Error(String(e)), this.loc, this.description));
      }
    }
  }
  ...
  getAnnotations(): Array<[Syntax.SourceLocation, Array<JSVal>, JSVal | undefined]> {
    return this.getInterpreter()
      .annotations.filter((annotation) => annotation.location.file === getOptions().filename)
  ...
  private solveLocal(smt: SMTInput): Promise<SMTOutput> {
    ...
    if (!checkedLocalZ3Version) {
      p = p.then(
        () =>
          new Promise<SMTOutput>((resolve, reject) => {
            ...
            exec(getOptions().z3path + ' -version', (err: Error, out: string) => {
              if (err) {
                ...
                if (!vstr || +vstr[1] < 4 || (+vstr[1] === 4 && +vstr[2] < 6)) {
                  reject(new Error('esverify requires z3 version 4.6 or higher'));
                } else {
      ...
      p = p.then(
        () =>
          new Promise<string>((resolve, reject) => {
            ...
            writeFile(getOptions().logsmt, smt, (err: Error, _out: string) => {
              if (err) {
    ...
    p = p.then(
      () =>
        new Promise<SMTOutput>((resolve, reject) => {
          ...
          const p = spawn(getOptions().z3path, [`-T:${getOptions().timeout}`, '-smt2', '-in'], {
